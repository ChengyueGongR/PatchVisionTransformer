...
def multi_mix(samples, targets, num_patch=14, num_mix=3):
    mix_rate = [1.] * num_mix
    mix_rate = np.array(mix_rate) + np.random.randint(0, 2, size=num_mix)
    mix_rate /= mix_rate.sum()
    # mix_rate = [1. / (num_mix + 0.)] * num_mix
    ...
    _new_samples = repeat_samples * mask_lst[0].reshape(1, 1, num_patch, 1, num_patch, 1)
    _new_samples += sum([new_samples[img_index_lst[_]] * mask_lst[_].reshape(1, 1, num_patch, 1, num_patch, 1)  for _ in range(1, num_mix)])
    new_samples = _new_samples.reshape(num_batch, num_channel, img_size, img_size)
    new_targets = repeat_targets * mix_rate[0]
    new_targets += sum([mix_rate[_] * targets[img_index_lst[_]] for _ in range(1, num_mix)])
    
    return new_samples, new_targets, mix_rate, ([repeat_targets] + [targets[img_index_lst[_]] for _ in range(1, num_mix)], mask_lst, new_targets)


def repeat_two_mix(samples, targets, num_patch=14, repeat=2):
    mask = torch.rand(num_patch, num_patch).cuda()
    lam = np.random.beta(1., 1.)
    # lam = max(max(lam, 1 - lam), 0.9)
...
def two_mix(samples, targets, num_patch=14, local_consist=7):
    ...
    return new_samples, new_targets, mix_rate, ([targets, targets[img_index]], [mask, 1 - mask], img_index)

def train_one_epoch(model: torch.nn.Module, criterion: DistillationLoss,
                    data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, loss_scaler, max_norm: float = 0,
                    model_ema: Optional[ModelEma] = None, mixup_fn: Optional[Mixup] = None, teacher=None, set_training_mode=True):
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            loss = torch.sum(-targets * (1e-8 + outputs.softmax(dim=-1)).log(), dim=-1).mean()
