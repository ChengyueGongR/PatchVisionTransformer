...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., groups=1):
        ...
        # test: flops-friendly version: channel-shuffle + group
        self.groups = groups
        if self.groups != 1:
            ...
            self.fc1 = torch.nn.Conv1d(in_features, hidden_features, groups=self.groups, kernel_size=1) 
            self.fc2 = torch.nn.Conv1d(hidden_features, out_features, groups=self.groups, kernel_size=1)
        else:
